...
set LOG=logs\rank.log

echo.>>"%LOG%"
(echo ===== %DATE% %TIME% start =====)>>"%LOG%"

rem npm.cmd のパスにはスペースが含まれるので、必ず引用符で囲む
call "C:\Program Files\nodejs\npm.cmd" run rank:collect >>"%LOG%" 2>&1
set CODE=%ERRORLEVEL%

rem 数字の直後に >> を書くと cmd がストリーム番号と勘違いするため、括弧で囲む
